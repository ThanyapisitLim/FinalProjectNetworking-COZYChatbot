...
router.post("/", verifyToken, async (req: AuthRequest, res) => {
    try {
        ...
        if (!role || !context) {
            return res.status(400).json({ error: "role and context are required" });
        }
...
router.get("/", verifyToken, async (req: AuthRequest, res) => {
    try {
        ...
        if (cachedMessages) {
            return res.status(200).json({ data: JSON.parse(cachedMessages) });
        }
...
router.delete("/group/:groupId", verifyToken, async (req: AuthRequest, res) => {
    try {
        ...
        return res.status(200).json({ message: "Group and its messages deleted successfully" });
